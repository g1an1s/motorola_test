...
/*class Parser {
 public:
  ...
  bool TryParseOne(ngcp::test::CreateRadioProfileRequest* profile) {
    ...
    long temp = 0;
    ReadLong(&temp);
    profile->mutable_location()->set_latitude(temp);
    if (!Match(",") || !Match(longitude_)) {
      return SetFailedAndReturnFalse();
    }
    ReadLong(&temp);
    profile->mutable_location()->set_longitude(temp);
    ...
    profile->set_name(db_.substr(name_start, current_-name_start-1));
    if (!Match("},")) {
...
/*void ParseDb(const std::string& db, std::vector<ngcp::test::CreateRadioProfileRequest>* profile_list) {
  ...
  ngcp::test::CreateRadioProfileRequest profile;
  while (!parser.Finished()) {
    profile_list->push_back(CreateRadioProfileRequest());
    if (!parser.TryParseOne(&profile_list->back())) {
      std::cout << "Error parsing the db file";
      profile_list->clear();
      break;
    }
  }
  std::cout << "DB parsed, loaded " << profile_list->size()
            << " profiles." << std::endl;
}*/
